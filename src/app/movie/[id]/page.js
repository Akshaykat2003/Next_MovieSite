import React from 'react';
import styles from "@/app/styles/common.module.css";
import Image from "next/image";
import Link from "next/link";
import poster from "/public/poster.jpg"


const Page = async ({ params }) => {
    const id = params.id;

    const url = `https://netflix54.p.rapidapi.com/title/details/?ids=${id}&lang=en`;
    const options = {
        method: 'GET',
        headers: {
            'x-rapidapi-key': 'ecaaaf494bmshee3d5f93f9a537ep12bd4ajsn86bf384d4d59',
            'x-rapidapi-host': 'netflix54.p.rapidapi.com'
        }
    };

    try {
        const res = await fetch(url, options);
        const data = await res.json();
        const main_data = data[0].details;

        console.log(main_data); // Log the data for debugging

        const imageUrl = main_data.backgroundImage?.url;

        return (
            <div className={styles.container}>
                <h2 className={styles.movie_title}> Netflix \ <span>{main_data.type}</span></h2>
                <div className={styles.card_section}>

                    <div>
                        {imageUrl ? (
                            <Image src={imageUrl} alt={main_data.title} width={600} height={300} />
                        ) : (
                            <Image src={poster} alt="Default Image" width={600} height={300} />
                        )}
                    </div>



                    <div>
                        <h1>{main_data.title}</h1>
                        <p>{main_data.synopsis}</p>
                    </div>
                </div>
            </div>
        );
    } catch (error) {
        console.error("Error fetching data:", error);
        return <div>Error loading page.</div>;
    }
};

export default Page;
